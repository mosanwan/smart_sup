...
- 空档原地掉头时，Android App 使用动态正反推差值：航向误差超过容差后，左右总差值随误差从默认 `10%` 增加到 `60%`，可在设置页调整“空档最小转向差/最大转向差”。该逻辑只作用于空档锁航/角度转向，非空档前进锁航仍使用比例差速修正。
- 持续航向锁定带 3 秒误差窗口自适应补偿：同向误差长期不收敛时逐步增加差速，误差改善或变号时自动衰减，补偿上限为额外 `50%` 且仍受最终输出限幅保护。

...
- 航向锁定和角度转向默认使用 Android 手机指南针航向，由 App 本地计算目标航向、误差和左右 ESC 功率；设置页提供“航向锁定使用 ESP32 IMU”测试开关，可改用新九轴 IMU 模块内置算法输出的欧拉角 `YBY`。Android 只保留 `YBY 原始` / `YBY 反向` 两种方向选择，并提供“设为正北 0°”把当前 `YBY` 保存为零点偏置；“对齐手机航向”仍可用手机指南针校准偏置。切换来源、切换算法或校准时取消当前锁航并回空挡。ESP32 不独立做航向闭环，只执行最终左右功率并保留解锁、失联、限幅和 PWM 安全保护。手机指南针模式要求手机固定在板体上且相对船体方向不变。

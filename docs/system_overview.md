...
- 航向锁定和角度转向默认使用 Android 手机指南针航向，由 App 本地计算目标航向、误差和左右 ESC 功率；设置页提供“航向锁定使用 ESP32 IMU”测试开关，可改用新九轴 IMU 航向。由于实物安装方向和模块四元数坐标约定未完全确认，Android 设置页提供 `YBY` 正反、四元数正/反向、船头轴 `+/-X`/`+/-Y`、以及世界坐标 `XY` 交换的候选算法；用户现场选择与手机航向同向变化且角度接近的算法后，再用“对齐手机航向”只校准 `offset`。切换来源、切换算法或校准时取消当前锁航并回空挡。ESP32 不独立做航向闭环，只执行最终左右功率并保留解锁、失联、限幅和 PWM 安全保护。手机指南针模式要求手机固定在板体上且相对船体方向不变。

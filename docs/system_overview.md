...
- 航向锁定、角度转向和自动导航航向修正共用 ESP32 固件里的同一套航向转向内环；定点保持使用 ESP32 专用 `MODE=STATION_KEEP` 位置闭环。Android App 只负责记录/更新目标航向、目标坐标、当前坐标、基础推力、限幅参数、航向源和控制保活，不在本地计算最终左右 ESC 功率。锁航启动、取消或目标/基础推力变化时发送 `MODE=HEADING_LOCK` 事件；定点保持发送 `TLAT/TLON/CLAT/CLON/SLIM`。固件在 `20ms` PWM 控制周期内使用当前选择的航向源闭环或坐标转换：默认主控 IMU `YBHDG/YBGZ`；手机模式由 Android 发送校准后的 `PHDG`。最终差速、扰动补偿、预测反打、限幅和左右 ESC 输出仍全部由 ESP32 固件计算。

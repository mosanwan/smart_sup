...
- 航向锁定和角度转向默认使用 Android 手机指南针航向，由 App 本地计算目标航向、误差和左右 ESC 功率；设置页提供“航向锁定使用 ESP32 IMU”测试开关，可改用新九轴 IMU 上报的 `YBY`。当前模块芯片面朝下安装，Android 端按 `heading = normalize(YBY + offset)` 转换为船体航向，设置页可用手机当前航向校准 `offset`；切换来源或校准时取消当前锁航并回空挡。ESP32 不独立做航向闭环，只执行最终左右功率并保留解锁、失联、限幅和 PWM 安全保护。手机指南针模式要求手机固定在板体上且相对船体方向不变。自动导航仍要求手机指南针可用。
- 空档原地掉头时，Android App 使用动态正反推差值：航向误差超过容差后，左右总差值随误差从默认 `20%` 增加到 `60%`，可在设置页调整“空档最小转向差/最大转向差”；`20%` 总差值约等于单侧 `10%`，用于避开当前电机起转死区。该逻辑只作用于空档锁航/角度转向，非空档前进锁航仍使用比例差速修正。
- 持续航向锁定带 3 秒误差窗口自适应补偿：同向误差长期不收敛时逐步增加差速，误差同向改善但仍未进容差时保持补偿，误差回到容差或变号时自动衰减，补偿上限为额外 `50%` 且仍受最终输出限幅保护。

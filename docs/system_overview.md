...
- 航向锁定和角度转向默认使用 Android 手机指南针航向，由 App 本地计算目标航向、误差和左右 ESC 功率；设置页提供“航向锁定使用 ESP32 IMU”测试开关，可改用新九轴 IMU 上报的四元数航向。当前模块芯片面朝下安装，Android 端把 IMU 船头方向向量经四元数旋转后投影到水平面计算船体航向，设置页可用手机当前航向校准 `offset`；四元数无效时才用 `YBY` 兜底。切换来源或校准时取消当前锁航并回空挡。ESP32 不独立做航向闭环，只执行最终左右功率并保留解锁、失联、限幅和 PWM 安全保护。手机指南针模式要求手机固定在板体上且相对船体方向不变。自动导航仍要求手机指南针可用。

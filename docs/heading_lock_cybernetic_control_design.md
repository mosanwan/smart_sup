...
| ESP32 固件 | 读取当前航向源、计算 `e/r/r_dot/e_pred`、执行航向外环、角速度内环、短时扰动补偿和动态 `BRAKE`、输出左右 ESC |
...
- 固件锁航可使用主控 IMU 或手机指南针作为航向源。主控 IMU 模式要求 `YBIMU=1` 且 `YBHDG/YBGZ` 新鲜有效；手机模式要求 Android 持续上报校准后的 `PHDG`，固件用连续 `PHDG` 差分估计 `r`。
- 手机指南针进入闭环时，手机必须固定在桨板上且相对船体方向不变；`PHDG` 超过 `1.5s` 未更新时，固件暂停差速修正并等待新读数，不能按旧航向继续积分补偿。
- 心跳超时、航向源超时、故障或用户取消时，固件必须停止推进差速输出并回空挡或锁定；普通航向源超时不得无授权取消用户锁航目标。
...
当前推进器语义约定：目标在左侧、误差为负时，需要右推进增加、左推进减少；目标在右侧、误差为正时，需要左推进增加、右推进减少。该符号只在 ESP32 固件锁航转向内环到语义 `L/R` 输出这一步处理，ESC 物理方向反转仍只允许在最终 PWM 映射层处理。
...
2026-06-27 阻尼测试版把稳定性放在响应速度前面：目标角速度上限降到约 `28°/s`，角速度内环增益降到 `0.45%/(°/s)`，`HBOOST` 上限降到 `35%`，爬升限制降到 `8%/s`，最低爬升降到 `2.5%/s`。当 `HBOOST` 符号和当前角速度误差相反时，先按 `0.80` 快速泄放再积分，避免过零后旧积分继续把船推过头。普通纠偏命令还增加命令层斜率限制，约 `22%/s`；预测过冲进入 `BRAKE` 时才允许更快的约 `85%/s` 反打命令。
...
| 参数 | 初始值 |
| --- | --- |
| `minBrakeRateDegS` | `5°/s` |
| `fullBrakeRateDegS` | `25°/s` |
| `minEffectiveBrakePercent` | `22%` |
| `maxBrakePercent` | `58%` |

因为电机 `9%` 才刚起转，反打不能落在 `10%-15%` 这种死区边缘。有基础推力时，只有预测会过冲且角速度仍偏大，`BRAKE` 才允许短时间跨过基础推力进入反推；普通纠偏仍优先减小差速和消耗积分，不靠持续反推转向。
...
| 参数 | 初始值 |
| --- | --- |
| `minBrakeHoldMs` | `350ms` |
| `maxBrakeHoldMs` | `1100ms` |
